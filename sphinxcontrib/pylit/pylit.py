...
class PylitListProcessor:
    ...
    def process(self, doctree: nodes.document, docname: str) -> None:
        ...
        for node in doctree.traverse(pylitlist):
            ...
            for pylit in pylits:
                ...
                document += new_pylit
                self.env.resolve_references(document, pylit['docname'], self.builder)
    ...
    def create_pylit_reference(self, pylit: pylit_node, docname: str) -> nodes.paragraph:
        ...
        try:
            reference['refuri'] = self.builder.get_relative_uri(docname, pylit['docname'])
...
def purge_pylits(app: Sphinx, env: BuildEnvironment, docname: str) -> None:
    warnings.warn('purge_pylits() is deprecated.', RemovedInSphinx40Warning, stacklevel=2)
    env.pylit_all_pylits = [pylit for pylit in env.pylit_all_pylits  # type: ignore
                          if pylit['docname'] != docname]
...
def merge_info(app: Sphinx, env: BuildEnvironment, docnames: Iterable[str],
    ...
    if not hasattr(other, 'pylit_all_pylits'):
        return
    if not hasattr(env, 'pylit_all_pylits'):
        env.pylit_all_pylits = []  # type: ignore
    env.pylit_all_pylits.extend(other.pylit_all_pylits)  # type: ignore
